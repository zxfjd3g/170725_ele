  ...
  export default {
    ...
    data () {
      return { // true 'true'  false 'false'
        favorite: localStorage.getItem('ele_favorite')==='true',
        supportClasses: ['decrease', 'discount', 'guarantee', 'invoice', 'special']
      }
    },

    mounted () {
      console.log('seller mounted()', this.seller)
      if(!this.seller.name) { // 如果没有数据, 直接结束(没有创建scroll对象)
        return
      }
    ...
    methods: {
      toggleFavorite () {
        ...
        // 保存
        localStorage.setItem('ele_favorite', this.favorite)
      },


      _initScroll () {
        if(!this.sellerScroll) { // 在当前路由刷新
          this.$nextTick(() => {
            ...
            const liWidth = 120
            const space = 6
            const width = (liWidth + space)*this.seller.pics.length - space
